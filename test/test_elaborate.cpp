...
START_TEST(test_elab1)
{
   ...
   ck_assert_int_eq(1, sx->nets().size());
   ck_assert_int_eq(0, sx->nets()[0]->nid());

   Signal *sy = root->signals()[1];
   ck_assert_ident_eq("Y", sy->name());
   ck_assert_int_eq(1, sy->nets().size());
   ck_assert_int_eq(1, sy->nets()[0]->nid());

   Scope *child = root->children()[0];
...
extern "C" Suite *get_elaborate_tests(void)
{
   Suite *s = suite_create("elaborate");
